...
#        self.acc = []
    ...
    def inference_interpolate(self, emotion_global, emotion_local, alpha, text, maxlen=1000, threshold=.5, ref_wav=True):
        text = self.pos_txt(self.text_norm(self.text_embedding(text)))
        text = text.transpose(0, 1) #T, B, C
        mels = torch.zeros((1, 1, self.nmels), device=text.device) #B, T, C, B=1
        attn_mask = torch.zeros((self.nheads, 1, text.size(0)), device=text.device)
        ...
        if ref_wav:
            ...
            if exists('l_emo_embedding_0.txt') and exists('l_emo_embedding_1.txt'):
                emotion_local_numpy_0 = np.loadtxt("l_emo_embedding_0.txt").reshape(1, 100, 256)
                emotion_local_0 = torch.from_numpy(emotion_local_numpy_0).float().to(text.device)
                emotion_local_numpy_1 = np.loadtxt("l_emo_embedding_1.txt").reshape(1, 100, 256)
                emotion_local_1 = torch.from_numpy(emotion_local_numpy_1).float().to(text.device)
                emotion_local = torch.mul(alpha, emotion_local_0) + torch.mul((1-alpha), emotion_local_1)
            else:
                #Because emotion_local went through LSTM model, the output becomes fixed size [100,32]
                # print("the l_emo size is: "+str(emotion_local.size()))
                print("No existing embedding, currently use random l_emo to generate it")
                emotion_local = torch.matmul(emotion_local, keys)
                #The key size is [1, 32, 256]
                # print("the key size is: "+str(keys.size()))
                #The embedding size is [1, 100, 256]
                #print("the emotion_local size is: "+str(emotion_local.size()))

                emotion_local_save = emotion_local.cpu().detach().numpy()
                embedding_file = open("l_emo_embedding.txt", "w")
                for row in emotion_local_save:
                    np.savetxt(embedding_file, row)
