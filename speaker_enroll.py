...
# Example usage: python3 custom_enroll.py FAKEBOB/data/z-norm/
...
from FAKEBOB.ivector_PLDA_kaldiHelper import ivector_PLDA_kaldiHelper
from FAKEBOB.gmm_ubm_kaldiHelper import gmm_ubm_kaldiHelper
...
enroll_dir = "FAKEBOB/data/enrollment-set"  # voice data for enrollment
z_norm_dir = "FAKEBOB/data/z-norm/"  # voice data for z norm
pre_model_dir = "FAKEBOB/pre-models"
model_dir = "FAKEBOB/model"
if not os.path.exists(model_dir):
    os.makedirs(model_dir)

audio_dir = os.path.abspath("FAKEBOB/audio-build-model-iv")
if not os.path.exists(audio_dir):
    os.makedirs(audio_dir)
mfcc_dir = os.path.abspath("FAKEBOB/mfcc-build-model-iv")
if not os.path.exists(mfcc_dir):
    os.makedirs(mfcc_dir)
log_dir = os.path.abspath("FAKEBOB/log-build-model-iv")
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
ivector_dir = os.path.abspath("FAKEBOB/ivector-build-model-iv")
if not os.path.exists(ivector_dir):
    os.makedirs(ivector_dir)

audio_dir_gmm = os.path.abspath("FAKEBOB/audio-build-model-gmm")
if not os.path.exists(audio_dir_gmm):
    os.makedirs(audio_dir_gmm)
mfcc_dir_gmm = os.path.abspath("FAKEBOB/mfcc-build-model-gmm")
if not os.path.exists(mfcc_dir_gmm):
    os.makedirs(mfcc_dir_gmm)
log_dir_gmm = os.path.abspath("FAKEBOB/log-build-model-gmm")
if not os.path.exists(log_dir_gmm):
    os.makedirs(log_dir_gmm)
score_dir = os.path.abspath("FAKEBOB/score-build-model-gmm")
...
z_norm_spk_id = []
z_norm_utt_path = []
for root, dirs, files in os.walk(z_norm_dir):
    for audio_name in files:
        utt_id = audio_name.split(".")[0]
        spk_id = utt_id.split("-")[0]
        path = os.path.join(root, audio_name)
        z_norm_utt_path.append(path)
        z_norm_utt_id.append(utt_id)
        z_norm_spk_id.append(spk_id)
...
iv_helper = ivector_PLDA_kaldiHelper(audio_dir=audio_dir, pre_model_dir = pre_model_dir, mfcc_dir=mfcc_dir, log_dir=log_dir, ivector_dir=ivector_dir)
